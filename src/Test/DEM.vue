<template>
  <div class="social-media-app">
    ...
    <nav class="navbar">
      ...
      <h2 class="section-title">QQ开发者社区 - Markdown使用规范调整</h2>
...
import { getPosts, searchComments, sendPosts, submitComments, updatePosts } from '@/router/user'
      ...
      users: [
        {
          ...
          username: 'Alice',
          bio: '喜欢编程和旅行，热衷于探索新技术。'
        },
        {
          id: 2,
          avatar: 'img/p2.jpg',
          username: 'Bob',
          bio: '热爱音乐和艺术，喜欢分享自己的创作。'
        },
        {
          id: 3,
          avatar: 'img/p3.jpg',
          username: 'Charlie',
          bio: '对科技和运动都充满热情，经常参加马拉松比赛。'
        }
      ],
      posts: [],
    ...
    async load () {
      try {
        ...
        if (Array.isArray(postsResponse.data) && postsResponse.data.length > 0) {
          const postsPromises = postsResponse.data.map(async (post) => {
            const dateObj = new Date(post.date)
            dateObj.setDate(dateObj.getDate() + 1)
            const newDateStr = dateObj.toISOString().split('T')[0]
            const newTime = `${newDateStr} ${post.time}`

            const isLiked = localStorage.getItem(`post-${post.id}-isLiked`) === 'true'

            // 获取评论
            const commentsResponse = await searchComments(post.id)
            const comments = (commentsResponse.data || []).map((comment) => {
              const commentDateObj = new Date(comment.date)
              commentDateObj.setDate(commentDateObj.getDate() + 1)
              const commentNewDateStr = commentDateObj.toISOString().split('T')[0]
              const commentNewTime = `${commentNewDateStr} ${comment.time}`
              ...
              return {
                ...
                time: commentNewTime,
                likeCount: comment.dianzan,
                fenxiang: comment.fenxiang,
                zhuanfa: comment.zhuanfa
              }
            })

            return {
              id: post.id,
              title: 'null', // 假设这里应该是从post中获取的标题，但您给的是'null'
              author: post.username,
              date: newTime,
              content: post.content,
              likeCount: post.dianzan,
              fenxiang: post.fenxiang,
              zhuanfa: post.zhuanfa,
              commentContent: '',
